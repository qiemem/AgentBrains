...
to setup [ layer-counts ]
  ...
  (foreach (but-first layers) (but-last layers) [
    ask ?1 [
      create-links-from ?2 [
        set-weight 0
      ]
...
to-report apply-bools [ input ]
  let result apply-reals map [ ifelse-value ? [1][0] ] input
  report map [? > 0.5] result
end
  
to-report apply-reals [ input ]
  set-input input
  propogate
  report map [ [ activation ] of ? ] output-node-list
end

to set-input [ input ]
  (foreach input-node-list input [
...
to-report get-output
  report map [ [activation] of ? > 0.5 ] output-node-list
end

to propogate
  foreach but-first layers [
    ask ? [ update-activation ]
  ]
end
...
to set-weights [ weights ]
  (foreach (sort links) (weights) [
    ask ?1 [ set-weight ?2 ]
  ])
end

to set-biases [ biases ]
  (foreach (sort turtles) (biases) [
    ask ?1 [ set-bias ?2 ]
  ])
end

to-report get-weights
  report map [ [weight] of ? ] sort links
end

to-report get-biases
  report map [[bias] of ?] sort turtles
end
