...
to-report sense
  let in (map runresult inputs)
  report (ls:report brain "apply-bools ?" in)
end
...
to reproduce
  if random 100 < reproduce-% [
    ...
    let child-weights map [ ? + random-normal 0 0.05 ] "get-weights" ls:of brain
    let child-biases map [ ? + random-normal 0 0.05 ] "get-biases" ls:of brain
    hatch 1 [
      setup-brain
      (ls:ask brain "set-weights ?1 set-biases ?2" child-weights child-biases)
